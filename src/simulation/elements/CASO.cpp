...
int CASO_update(UPDATE_FUNC_ARGS)
{
	/*
	int temp = 0;
	int oxyposnum = 0;
	...
	for (int rx = -2; rx <= 2; rx++)
		for (int ry = -2; ry <= 2; ry++)
			if (BOUNDS_CHECK && (rx || ry))
			{
				...
				if (!r || TYP(r) == PT_O2 || TYP(r) == PT_LO2 || TYP(r) == PT_CASO)
				{
				//	parts[i].containsoxy++;
...
void CASO_create(ELEMENT_CREATE_FUNC_ARGS)
{
...
void CASO_init_element(ELEMENT_INIT_FUNC_ARGS)
{
	elem->Identifier = "DEFAULT_PT_CASO";
	elem->Name = "CaSO4";
	elem->Colour = COLPACK(0xECECEC);
	elem->MenuVisible = 1;
	...
	elem->Weight = (int) (2.32 * 20);

	elem->HeatConduct = 150;
	elem->Latent = 0;
	elem->Description = "Calcium sulfate. Occurs naturally as the mineral gypsum.";
	...
	elem->HighTemperatureTransitionThreshold = 1460.0;
	elem->HighTemperatureTransitionElement = PT_LAVA;


	elem->Update = &CASO_update;
	elem->Func_Create = &CASO_create;
	//elem->Graphics = &COAL_graphics;
	elem->Init = &CASO_init_element;
}
