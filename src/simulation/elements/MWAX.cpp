...
int MWAX_update(UPDATE_FUNC_ARGS)
{

	
		if (parts[i].temp <= ((RNG::Ref().between(5, 20) + 45 + 273.15) + 273.15) + (sim->air->pv[(y) / CELL][(x) / CELL]) && RNG::Ref().chance(1, 1000))
		{
...
void MWAX_create(ELEMENT_CREATE_FUNC_ARGS)
{
...
void MWAX_init_element(ELEMENT_INIT_FUNC_ARGS)
{
	...
	elem->Update = MWAX_update;
	elem->Func_Create = &MWAX_create;
