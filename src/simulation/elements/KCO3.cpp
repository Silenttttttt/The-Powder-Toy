...
int KCO3_update(UPDATE_FUNC_ARGS)
{
	/*
	int temp = 0;
	int oxyposnum = 0;
	...
	for (int rx = -2; rx <= 2; rx++)
		for (int ry = -2; ry <= 2; ry++)
			if (BOUNDS_CHECK && (rx || ry))
			{
				...
				if (!r || TYP(r) == PT_O2 || TYP(r) == PT_LO2 || TYP(r) == PT_KCO3)
				{
				//	parts[i].containsoxy++;
...
int COAL_graphics(GRAPHICS_FUNC_ARGS);
void KCO3_create(ELEMENT_CREATE_FUNC_ARGS)
{
...
void KCO3_init_element(ELEMENT_INIT_FUNC_ARGS)
{
	elem->Identifier = "DEFAULT_PT_KCO3";
	elem->Name = "KCO3";
	...
	elem->Weight = (int) (2.43 * 20);

	elem->HeatConduct = 150;
	elem->Latent = 0;
	elem->Description = "Potassium carbonate?.";
	...
	elem->Update = &KCO3_update;
	elem->Func_Create = &KCO3_create;
	elem->Graphics = &COAL_graphics;
	elem->Init = &KCO3_init_element;
}
