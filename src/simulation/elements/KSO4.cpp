...
int KSO4_update(UPDATE_FUNC_ARGS)
{
	/*
	int temp = 0;
	int oxyposnum = 0;
	...
	for (int rx = -2; rx <= 2; rx++)
		for (int ry = -2; ry <= 2; ry++)
			if (BOUNDS_CHECK && (rx || ry))
			{
				...
				if (!r || TYP(r) == PT_O2 || TYP(r) == PT_LO2 || TYP(r) == PT_KSO4)
				{
				//	parts[i].containsoxy++;
...
void KSO4_create(ELEMENT_CREATE_FUNC_ARGS)
{
...
void KSO4_init_element(ELEMENT_INIT_FUNC_ARGS)
{
	elem->Identifier = "DEFAULT_PT_KSO4";
	elem->Name = "K2SO4";
	...
	elem->Latent = 0;
	elem->Description = "Potassium sulfate. a common oxidizer, ingredient in blackpowder.";
	...
	elem->Update = &KSO4_update;
	elem->Func_Create = &KSO4_create;
	//elem->Graphics = &COAL_graphics;
	elem->Init = &KSO4_init_element;
}
