...
void S_create(ELEMENT_CREATE_FUNC_ARGS)
{
...
void S_init_element(ELEMENT_INIT_FUNC_ARGS)
{
	elem->Identifier = "DEFAULT_PT_S";
	elem->Name = "S";
	elem->Colour = COLPACK(0xE7E146);
	elem->MenuVisible = 1;
	...
	elem->Weight = (int) (2 * 20);

	elem->HeatConduct = 150;
	elem->Latent = 0;
	elem->Description = "Elemental sulfur. Smells bad and burns slowly.";
	...
	elem->Update = &S_update;
	elem->Func_Create = &S_create;
	elem->Graphics = &COAL_graphics;
	elem->Init = &S_init_element;
}
