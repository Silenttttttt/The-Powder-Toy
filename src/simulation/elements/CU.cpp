...
int CU_update(UPDATE_FUNC_ARGS)
{
	/*
	int temp = 0;
	int oxyposnum = 0;
	...
	for (int rx = -2; rx <= 2; rx++)
		for (int ry = -2; ry <= 2; ry++)
			if (BOUNDS_CHECK && (rx || ry))
			{
				...
				if (!r || TYP(r) == PT_O2 || TYP(r) == PT_LO2 || TYP(r) == PT_CU)
				{
				//	parts[i].containsoxy++;
...
void CU_create(ELEMENT_CREATE_FUNC_ARGS)
{
...
void CU_init_element(ELEMENT_INIT_FUNC_ARGS)
{
	...
	elem->Weight = (int) (8.96 * 20);

	elem->HeatConduct = 301;
	elem->Latent = 0;
	...
	elem->Update = &CU_update;
	elem->Func_Create = &CU_create;
	//elem->Graphics = &COAL_graphics;
	elem->Init = &CU_init_element;
}
