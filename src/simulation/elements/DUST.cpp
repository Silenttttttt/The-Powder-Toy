...
int DUST_update(UPDATE_FUNC_ARGS)
{
	...
	if (parts[i].life <= 0)
	{
		...
		parts[i].ctype = temp;
		parts[i].life = RNG::Ref().between(30, 50);
		return 1;
	}
...
void DUST_create(ELEMENT_CREATE_FUNC_ARGS)
{
...
void DUST_init_element(ELEMENT_INIT_FUNC_ARGS)
{
	...
	elem->Update = &DUST_update;
	elem->Func_Create = &DUST_create;
}
