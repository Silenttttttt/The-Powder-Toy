...
int CA_update(UPDATE_FUNC_ARGS)
{
	/*
	int temp = 0;
	int oxyposnum = 0;
	...
	for (int rx = -2; rx <= 2; rx++)
		for (int ry = -2; ry <= 2; ry++)
			if (BOUNDS_CHECK && (rx || ry))
			{
				...
				if (!r || TYP(r) == PT_O2 || TYP(r) == PT_LO2 || TYP(r) == PT_CA)
				{
				//	parts[i].containsoxy++;
...
void CA_create(ELEMENT_CREATE_FUNC_ARGS)
{
...
void CA_init_element(ELEMENT_INIT_FUNC_ARGS)
{
	...
	elem->Weight = (int) (1.55 * 20);

	elem->HeatConduct = 150;
	elem->Latent = 0;
	elem->Description = "Calcium metal.";
	...
	elem->Update = &CA_update;
	elem->Func_Create = &CA_create;
	//elem->Graphics = &COAL_graphics;
	elem->Init = &CA_init_element;
}
