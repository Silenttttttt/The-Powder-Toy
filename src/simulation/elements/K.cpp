	...
	for (int rx = -2; rx <= 2; rx++)
		for (int ry = -2; ry <= 2; ry++)
			if (BOUNDS_CHECK && (rx || ry))
			{
				...
				else if (RNG::Ref().chance(1, 10) && parts[i].life > 0 && oxyposnum <= 0 && (parts[i].temp > 377 + 273 - (sim->air->pv[(y) / CELL][(x) / CELL] / 4) || rt == PT_FIRE  || rt == PT_PLSM))
				{
...
void K_create(ELEMENT_CREATE_FUNC_ARGS)
{
...
void K_init_element(ELEMENT_INIT_FUNC_ARGS)
{
	elem->Identifier = "DEFAULT_PT_K";
	elem->Name = "K";
	elem->Colour = COLPACK(0x8e8e8e);
	elem->MenuVisible = 1;
	...
	elem->Weight = (int) (0.862 * 20);

	elem->HeatConduct = 150;
	elem->Latent = 0;
	elem->Description = "Potassium metal. Very reactive.";
	...
	elem->Update = &K_update;
	elem->Func_Create = &K_create;
	//elem->Graphics = &COAL_graphics;
	elem->Init = &K_init_element;
}
