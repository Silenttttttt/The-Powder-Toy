...
int HSO4_update(UPDATE_FUNC_ARGS)
{
	/*
	int temp = 0;
	int oxyposnum = 0;
	...
	for (int rx = -2; rx <= 2; rx++)
		for (int ry = -2; ry <= 2; ry++)
			if (BOUNDS_CHECK && (rx || ry))
			{
				...
				if (!r || TYP(r) == PT_O2 || TYP(r) == PT_LO2 || TYP(r) == PT_HSO4)
				{
				//	parts[i].containsoxy++;
...
void HSO4_create(ELEMENT_CREATE_FUNC_ARGS)
{
...
void HSO4_init_element(ELEMENT_INIT_FUNC_ARGS)
{
	elem->Identifier = "DEFAULT_PT_HSO4";
	elem->Name = "H2SO4";
	...
	elem->Weight = (int) (1.8 * 20);

	elem->HeatConduct = 150;
	elem->Latent = 0;
	elem->Description = "Sulfuric acid.";
	...
	elem->Update = &HSO4_update;
	elem->Func_Create = &HSO4_create;
	//elem->Graphics = &COAL_graphics;
	elem->Init = &HSO4_init_element;
}
