...
int DESL_update(UPDATE_FUNC_ARGS)
{

	if (parts[i].temp >= RNG::Ref().between(150, 380) + 273.15 - (sim->air->pv[(y) / CELL][(x) / CELL]) && RNG::Ref().chance(1, 1000) || RNG::Ref().chance(1, 100000))
	{
		parts[i].type = PT_GAS;
		parts[i].ctype = PT_DESL;
	}
	else if (parts[i].temp <= (RNG::Ref().between(5, 20) - 10) + (-23) + 273.15 && RNG::Ref().chance(1, 1000) || RNG::Ref().chance(1, 100000))
	{
		parts[i].type = PT_WAX;
		parts[i].ctype = PT_DESL;
	}
	...
	for (int rx = -2; rx <= 2; rx++)
		for (int ry = -2; ry <= 2; ry++)
			if (BOUNDS_CHECK && (rx || ry))
			{
				...
				if (RNG::Ref().chance(1, 10) && oxyposnum < 250 && parts[i].life > 0 && (parts[i].temp > 210 + 273 - (sim->air->pv[(y) / CELL][(x) / CELL] / 2) || TYP(r) == PT_FIRE || TYP(r) == PT_PLSM || TYP(r) == PT_EMBR) && (!r || (parts[ID(r)].oxidizer > 0 && parts[ID(r)].containsoxy > 0)))
				{
					//	parts[i].containsoxy++;
				...
				//continue;
				int rt = TYP(r);
				if ((RNG::Ref().chance(1, 5) && parts[i].life > 0) && (parts[i].containsoxy > 0 || oxyposnum > 0) && (parts[i].temp > 210 + 273 - (sim->air->pv[(y) / CELL][(x) / CELL] / 2) || rt == PT_FIRE || rt == PT_PLSM))
				{
					...
					//	int tempy = y + RNG::Ref().between(-1, 1);
						...
						if (RNG::Ref().chance(1, 10 - oxyposnum))
						{
							...
							if (np > -1)
							{
								...
								parts[np].life = RNG::Ref().between(20, 30);
				...
				else if (RNG::Ref().chance(1, 5) && parts[i].life > 0 && parts[i].containsoxy <= 0 && (parts[i].temp > 210 + 273 - (sim->air->pv[(y) / CELL][(x) / CELL] / 2) || rt == PT_FIRE || rt == PT_PLSM || rt == PT_EMBR))
				{
...
void DESL_create(ELEMENT_CREATE_FUNC_ARGS)
{
	parts[i].life = RNG::Ref().between(250, 400);
...
void DESL_init_element(ELEMENT_INIT_FUNC_ARGS)
{
	...
	elem->HeatConduct = 42;
	elem->Latent = 0;
	elem->Description = "Diesel. Hydrocarbon used in engines.";
	...
	elem->Update = &DESL_update;
	elem->Graphics = NULL;//&COAL_graphics;
	elem->Func_Create = &DESL_create;
